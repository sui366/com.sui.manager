...
PageList.prototype = {
	...
	toInertPage:function(){
		window.location.href= pathHelper_localUrl+"customer/service/updatePage.shtml?t="+new Date().getTime();
	},
	toUpdatePage:function(id){
		window.location.href= pathHelper_localUrl+"customer/service/updatePage.shtml?t="+new Date().getTime()+"&id="+id;
	},
	...
	deleteObj:function(id){
		...
		layer.confirm('确定要删除该条数据？', {
		  btn: ['确定','取消'] 
			...
			$.ajax({
				type : "post",
				url : pathHelper_localUrl+"customer/service/delete.action",
				data : {"id":id},
				complete:function(){
					layer.close(index);
				},
